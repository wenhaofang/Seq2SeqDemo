...
class Decoder(nn.Module):
    ...
    def forward(self, source, hidden, context):
        ...
        concated = torch.cat((embedded, context), dim = 2)
        ...
        if self.rnn_type == 'lstm':
            output = torch.cat((embedded.squeeze(0), hidden[0].squeeze(0), context.squeeze(0)), dim = 1)
...
class Seq2Seq(nn.Module):
    def __init__( self, rnn_type,
        ...
        assert enc_n_directions == dec_n_directions
...
def get_module(option, src_vocab_size, trg_vocab_size):
    seq2seq = Seq2Seq(option.rnn_type,
        src_vocab_size, option.enc_emb_dim, option.enc_hid_dim, option.enc_n_layers_group1, option.enc_n_directions_group1, option.enc_dropout,
